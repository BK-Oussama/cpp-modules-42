...
//         << " because their grade is too low (Grade " << this->getGrade()
//         << ", required " << form.getExecuteGrade() << ")." << std::endl;
//     }
...
//                   << " due to an unexpected error: " << e.what() << std::endl;
//     }
// }
...
void Bureaucrat::executeForm(AForm const & form) const
{
    ...
    catch (const std::exception& e)
    {
        ...
        //      1. AForm::GradeTooLowException
        //      2. std::logic_error (Form not signed)
        //      3. Any other std::exception
